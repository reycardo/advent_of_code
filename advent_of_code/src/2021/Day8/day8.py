    ...
    for entry in input:
        ...
        for word in entry[1].split():
            ...
            output_value = output_value + (
                [k for k, v in mds.items() if v == search_mds]
            )
...
def count_easy_digits(input):
    ...
    return len(
        [
            length
            for length in tools.flatten(output_length)
            if length in [len(mds[1]), len(mds[4]), len(mds[7]), len(mds[8])]
        ]
    )
