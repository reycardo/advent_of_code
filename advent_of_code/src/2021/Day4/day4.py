        ...
        for index, m_board in enumerate(m_boards):
            if index not in cleared:
                if np.isin(draw, m_board):
                    pos = np.where(
                        np.array(m_board) == int(draw)
                    )  # row and column of draw in board
                    m_board[
                        int(pos[0]), int(pos[1])
                    ] = -1  # if draw hits number gets replaced by -1
                    if check_bingo(m_board):
                        winners.append(m_board)  # append board to winners
                        checks.append(
                            np.count_nonzero(m_board < 0)
                        )  # append total of numbers hit to checks
...
def main(raw, part):
    ...
    if part == 1:
        return (winners[0].sum() + checks[0]) * int(
            last_draws[0]
        )  # sum of remaining digits in winning board * last draw
    elif part == 2:
        return (winners[-1].sum() + checks[-1]) * int(
            last_draws[-1]
        )  # sum of remaining digits in losing board * last draw
    else:
