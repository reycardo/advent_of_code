    ...
    def poli_counter(self, steps):
        for _ in range(steps):
            for (
                ...
                self.pair_count.copy().items()
            ):  # if pair = "CB" and count = 5 means there are 5 CB's across the poli
                added_letter = self.rules[
                    pair
                ]  # fetch the rule for pair "CB" to added_letter = "H"
                ...
                self.pair_count[pair[0] + added_letter] += (
                    count  # Add the amount of counts = 5 to the new pair generated "CH"
                )
                self.pair_count[added_letter + pair[1]] += (
                    count  # Add the amount of counts = 5 to the new pair generated "HB"
                )
                self.letter_count[added_letter] += (
                    count  # There are now 5 more entries of H in the polymer
                )
    ...
    def loop_steps(self, steps):
        while True:
            ...
            if self.step == steps:
                ...
                print(
                    f"Calm down with your exponential increments boy we are dealing with arrays of lenght: {len(self.poli)}"
                )
