...
class Tree_Grid:
    ...
    def is_visible(self, x, y):
        val = self.matrix[y][x]
        return (
            all(val > tree for tree in self.get_left(x, y))
            or all(val > tree for tree in self.get_right(x, y))
            or all(val > tree for tree in self.get_top(x, y))
            or all(val > tree for tree in self.get_bot(x, y))
        )
    ...
    def get_scenic_scores(self):
        return [
            self.check_scenic_score(x, y) for (x, y), val in np.ndenumerate(self.matrix)
        ]
