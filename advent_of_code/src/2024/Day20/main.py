...
class Puzzle:
    ...
    def find_new_cheats(self, point: Point, score_at_point: int):
        for i in range(score_at_point, len(self.original_trail[0][0])): # might need to change len
            ...
            if man_dist <= 20:            
                ...
                original_score = self.original_trail[0][0].index(next_point_2)
                new_score = score_at_point + man_dist + (len(self.original_trail[0][0] ) - 2 - original_score)
                time_diff = self.trail_time - new_score
                if time_diff not in self.saved_time:
    ...
    def solve(self, part):
        ...
        if part==1:
            ...
            return sum([count for saved_time, count in self.saved_time.items() if saved_time >= self.save])
...
def run_tests():
    ...
    assert main(raw=files["test"], part=2, save=50) == 32+31+29+39+25+23+20+19+12+14+12+22+4+3
