...
class Browse():
    ...
    def apply_command(self, string: str):
        if string.startswith('$ cd /'):
            ...
            nx.set_node_attributes(self.G, {name: size}, name='size')  # add size to file
            nx.set_node_attributes(self.G, {self.current_node: self.G.nodes[self.current_node]['size'] + size}, name='size')  # add size to current dir
            for parent in nx.ancestors(self.G, self.current_node):
                nx.set_node_attributes(self.G, {parent: self.G.nodes[parent]['size'] + size}, name='size') # add size to parent dirs
