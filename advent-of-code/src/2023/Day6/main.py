...
class Puzzle:
    ...
    def parse_input(self):
        parsed_time = [int(x) for x in self.input[0].split(":")[1].strip().split(" ") if x]
        parsed_distance = [int(x) for x in self.input[1].split(":")[1].strip().split(" ") if x]
        parsed_input_1 = list(zip(parsed_time, parsed_distance))
        parsed_input_2 = [(int(''.join(map(str, parsed_time))),int(''.join(map(str, parsed_distance))))]
    ...
    def run_all_distances(self, max_time):
        return [self.get_distance(time_pressed, max_time) for time_pressed in range(max_time)]        

    def run_pt2_distances(self):
        record_passed = False
        time_pressed = 0
        while not record_passed:
            if self.get_distance(time_pressed, self.input_parsed2[0][0]) > self.input_parsed2[0][1]:
                record_passed = True                        
                return time_pressed
            time_pressed += 1
