...
def is_inside(state: np.matrix,coords: tuple):
    if all((coords[0] >= 0, coords[0] < state.shape[0], coords[1] >= 0, coords[1] < state.shape[1])):
        return True
    else:
        return False
...
def main(raw, part):
    ...
    if part == 1:
        ...
        shortest_path = nx.single_source_dijkstra(G, source=start, target=target, weight='weight')
        #print(shortest_path)
