...
def draw_vents(vents,board,part):
    for vent in vents:
        ...
        if x1 == x2:
            ...
            for num_x,num_y in zip(range(min(x1,x2),max(x1,x2)+1),range(min(y1,y2),max(y1,y2)+1)):
                board[num_y,num_x] += 1
        elif part == 2 and ((y1 > y2 and x1 < x2) or (y1 < y2 and x1 > x2)): # 45º
            for num_x,num_y in zip(range(max(x1,x2),min(x1,x2)-1,-1),range(min(y1,y2),max(y1,y2)+1)):
                board[num_y,num_x] += 1                                   
    return board
